...
const Company = () => {
  ...
  useEffect(() => {
    window.addEventListener("keydown", openHandler);
    return () => window.removeEventListener("keydown", openHandler);
  }, []);
      ...
      {isOpen && (
        <div className={style.model}>
          <div className={style.tabs}>
            <span
              className={track === 0 && style.active}
              onClick={() => clickHandler(0)}
            >
              Detail
            </span>
            <span
              className={track === 1 && style.active}
              onClick={() => clickHandler(1)}
            >
              Customer Follow Up
            </span>
            <span
              className={track === 2 && style.active}
              onClick={() => clickHandler(2)}
            >
              Fund Available
            </span>
            <span
              className={track === 3 && style.active}
              onClick={() => clickHandler(3)}
            >
              Taking a Stand
            </span>
            <span
              className={track === 4 && style.active}
              onClick={() => clickHandler(4)}
            >
              Personalization of Investment
            </span>
            <span
              className={track === 5 && style.active}
              onClick={() => clickHandler(5)}
            >
              Banner & Blocks
            </span>
            <span
              className={track === 6 && style.active}
              onClick={() => clickHandler(6)}
            >
              Customer Push
            </span>
            <span
              className={track === 7 && style.active}
              onClick={() => clickHandler(7)}
            >
              Personal Doc
            </span>
            <span
              className={track === 8 && style.active}
              onClick={() => clickHandler(8)}
            >
              Dashboard
            </span>
            <span
              className={track === 9 && style.active}
